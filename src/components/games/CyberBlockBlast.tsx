  ...
  return (
    <div className="w-full max-w-xl mx-auto flex flex-col items-center select-none touch-manipulation transition-all duration-300">
      ...
      <div className="w-full grid grid-cols-4 gap-2 mb-3">
        ...
        {/* Combo */}
        <div className="p-3 rounded-2xl bg-zinc-100 dark:bg-zinc-900/80 border border-zinc-200 dark:border-zinc-800 text-center">
          <span className="text-[11px] font-medium text-zinc-500 dark:text-zinc-400 block">کمبو انفجار</span>
          <span className="text-lg font-black text-amber-500">
            {comboStreak > 1 ? `x${comboStreak}` : "آماده"}
          </span>
        </div>

        {/* Pieces Left */}
        <div className="p-3 rounded-2xl bg-zinc-100 dark:bg-zinc-900/80 border border-zinc-200 dark:border-zinc-800 text-center">
          <span className="text-[11px] font-medium text-zinc-500 dark:text-zinc-400 block">قطعات باقی</span>
          <span className="text-lg font-black text-cyan-500">{availableShapes.length} / ۳</span>
        </div>
      ...
      <div
        ...
        dir="ltr"
        className="relative p-3 sm:p-4 rounded-3xl bg-zinc-200/90 dark:bg-[#0d0d12] border-2 border-zinc-300 dark:border-zinc-800 shadow-2xl w-full aspect-square max-w-[390px] sm:max-w-[420px] flex items-center justify-center overflow-hidden"
      >
            ...
            row.map((cell, c) => {
              ...
              return (
                <button
                  ...
                  className={`relative rounded-lg transition-all duration-150 flex items-center justify-center border active:scale-95 cursor-pointer ${
                    isClearing
                ...
                >
                  {cell !== null && (
                    <div className="absolute top-0.5 left-0.5 right-0.5 h-1 bg-white/40 rounded-t-md pointer-events-none" />
                  )}
                </button>
              );
            })
        ...
        {/* Game Over Overlay */}
        {isGameOver && (
          <div className="absolute inset-0 bg-zinc-950/90 backdrop-blur-md rounded-3xl flex flex-col items-center justify-center p-6 text-center z-30 animate-fadeIn">
            ...
            <h3 className="text-xl font-black text-white mb-1">فضایی برای قطعات جدید نماند!</h3>
            <p className="text-sm font-bold text-emerald-400 mb-1">امتیاز نهایی: {score}</p>
            <p className="text-xs text-zinc-400 mb-6">شبکه را برای ثبت رکوردهای بالاتر آماده کنید.</p>
      ...
      <div
        dir="ltr"
        className="w-full max-w-[420px] mt-4 p-3 rounded-2xl bg-zinc-100/90 dark:bg-zinc-900/90 border-2 border-zinc-300/80 dark:border-zinc-800/80 shadow-xl flex items-center justify-around gap-2 min-h-[95px]"
      >
        {availableShapes.map((shape, idx) => {
          ...
          return (
            <button
              ...
              className={`relative p-3 rounded-xl transition-all duration-200 flex flex-col items-center justify-center cursor-grab active:cursor-grabbing touch-none select-none ${
                isCurrentlyDragged
                  ? "opacity-25 scale-95"
                  : isSelected
                  ? "bg-purple-500/20 border-2 border-purple-400 ring-2 ring-purple-400/40 shadow-lg -translate-y-1 scale-110"
        ...
        createPortal(
          ...
          >
            {(() => {
              ...
              return (
                <div
                  className="grid gap-1.5 p-2.5 rounded-2xl bg-zinc-950/90 backdrop-blur-md border-2 border-purple-400/90 shadow-[0_10px_30px_rgba(168,85,247,0.7)] scale-110"
                  style={{
      ...
      <div className="w-full max-w-[420px] flex items-center justify-between mt-3 px-1">
        {/* Undo */}
        <button
          type="button"
          onClick={handleUndo}
